...
$(function() {
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
        ...
        if (target == '#panel-carte') {
            ...
            {
                //console.log("first adresse " + dataSelected[i].adresse);
                coordonnees.push({
                    lat:dataSelected[i].latitude,
                    lng:dataSelected[i].longitude,
                    cp:dataSelected[i].cp,
                    name2:dataSelected[i].name,
                    adresseEtab2:dataSelected[i].adresse,
                    sigleEtab:dataSelected[i].sigle
                });
            }
...
function ajouteMarqueur( latlng , maCarte) {
    ...
    var optionsMarqueur = {
        ...
        icon:"/images/college2.png"
    };
    var marqueur = new google.maps.Marker( optionsMarqueur );

    //ajouer un évenement pour centrer la carte
    var infowindow = new google.maps.InfoWindow({
        content:getInfoMarker(latlng.name2, latlng.cp, latlng.adresseEtab2, latlng.sigleEtab)
    });
...
function getInfoMarker(nom, cp, adresse, sigle)
{
    cp=cp!=null?cp:"";
    //console.log("adresse " + adresse);
    adresse=adresse!=null?adresse:"";
    sigle=sigle!=null?sigle:"";
